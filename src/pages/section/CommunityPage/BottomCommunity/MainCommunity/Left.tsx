...
import PostNewStatusBarGroup from "../../../../../Components/PostNewStatusBarGroup";
...
const Right:React.FC<props>=({community,role})=>{
    ...
    const fetchPost = async (pageNum = 0) => {
      setLoading(true);
      const data: PaginatedResponse<PostDTO> = await PostService.getCommunityPosts(community.communityId,false, pageNum, 5);
      if (pageNum === 0) {
    ...
    return(
      <>
        <Row>
          <Col span={24}><PostNewStatusBarGroup communityId={community.communityId}/></Col>
        </Row>
